        ...
        <ul class="blog-listing detail">
          <li>
            <div class="image-zoom-container" @click="openImageModal">
              <img
                :src="buildSafeImageUrl(servicioData.serv_imagen)"
            ...
            <p class="left-aligned" v-html="$sanitize(servicioData.serv_descripcion)"></p>
...
export default {
  ...
  computed: {
    ...mapState(["servicios", "Institucion"]),
    ...
    servicioData() {
      ...
      if (!servicioId || !this.servicios?.length) return null;
      const servicio = this.servicios.find(s => s.serv_id === servicioId);
      if (!servicio || !servicio.serv_id || !servicio.serv_imagen) return null;
      return servicio;
    },
