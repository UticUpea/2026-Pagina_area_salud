<template>
        ...
        <ul class="blog-listing detail">
          <li>
            <div class="image-zoom-container" @click="openImageModal">
              <img
                :src="getDirectUrl(eventoData.evento_imagen)"
            ...
            <ul class="post-detail">
              ...
              <li>
                <span class="label">{{ eventoData.tipo_evento || 'Evento' }}</span>
              </li>
            </ul>
            
            <p class="left-aligned">Descripción del evento:</p>
            <p class="left-aligned" v-html="sanitizeHtml(eventoData.evento_descripcion)"></p>
  ...
  <div v-if="showImageModal" class="image-modal-overlay" @click="closeImageModal">
    <div class="image-modal-content" @click.stop>
      ...
      <img :src="getDirectUrl(eventoData.evento_imagen)" :alt="eventoData.evento_titulo" class="modal-image" />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    eventosList() {
      ...
      return vuexEventos.length ? vuexEventos : this.eventosLocal;
    },
    ...
    eventoData() {
      if (!this.eventoId || !this.eventosList.length) return null;
      
      const evento = this.eventosList.find(e => 
        e?.evento_id === this.eventoId &&
        e?.tipo_evento?.toUpperCase().trim() === 'INSTITUTO DE INVESTIGACION'
      );
      
      return (evento?.evento_id && evento.evento_imagen) ? evento : null;
    },

    eventoNotFound() {
      return this.eventosList.length > 0 && !this.eventoData;
    },
  },
  ...
  methods: {
    ...
    async fetchEventos() {
      try {
        ...
        this.eventosLocal = (res.data.upea_evento || [])
        ...
        if (process.env.NODE_ENV !== 'production') {
          console.error('[DetalleEventoInstituto] Error:', error.message);
        }
      } finally {
        this.loading = false;
      }
